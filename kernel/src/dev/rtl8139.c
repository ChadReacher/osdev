...
void rtl8139_init() {
	...
	u16 pci_command;
	rtl8139_device = pci_probe_device(RTL8139_VENDOR_ID, RTL8139_DEVICE_ID, -1);
	...
	tx_buffer_phys = (u32)virtual_to_physical((virtual_address)tx_buffer);

	/* Init receive buffer, set up additional spaces
	 * when frame overflows the buffer (8K) */
	rx_buffer = malloc(RX_BUFFER_SZ + 16 + 1500);
	rx_buffer_phys = (u32)virtual_to_physical((virtual_address)rx_buffer);
