...
void console_write(struct tty_struct *tty) {
    i32 count = CHARS(tty->output);
    while (count--) {
        if (tty->stopped) {
            break;
        }
        i8 c = ttyq_getchar(&tty->output);
        switch (state) {
            case NORMAL:
                if (c > 31 && c < 127) {
                    *(u16 *)pos = (c | (0x07 << 8));
...
void console_init(void) {
    physical_address paddr = 0xB8000;
    virtual_address vaddr = SCREEN_START;
    for (u32 i = 0; i < 0x8000; i += 0x1000) {
        map_page(paddr + i, vaddr + i, PAGING_FLAG_PRESENT | PAGING_FLAG_WRITEABLE);
    }
    ...
    debug("Console has been initialized\r\n");
}
