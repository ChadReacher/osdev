...
elf_header_t *elf_load(u32 *data) {
	...
	page_directory_t *cur_pd;
	page_directory_entry *stack_pd_entry;
	void *stack_phys_frame, *brk_phys_frame;
	page_table_t *stack_page_table;
	page_table_entry page_for_stack;


	elf = (elf_header_t *)data;
	...
	program_header = (elf_program_header_t*)((u32)data + elf->phoff);
	...
	stack_phys_frame = allocate_blocks(1);
	...
	*stack_pd_entry |= PAGING_FLAG_USER;
	...
	program_brk = program_header[0].vaddr + program_header[0].memsz;
	for (i = 1; i < elf->ph_num; ++i) {
...
void load_segment(u32 *data, elf_program_header_t *program_header) {
	u32 i, addr, len_in_blocks;
	void *code, *code_phys_frame;
	page_directory_t *cur_pd;
	...
	len_in_blocks = memsz / 4096;
	if (len_in_blocks % 4096 != 0 || len_in_blocks == 0) {
		++len_in_blocks;
	}

	code = (void *)((u32)data + offset);
	code_phys_frame = allocate_blocks(len_in_blocks);

	/* Map necessary pages for code */
	for (i = 0, addr = vaddr; i < len_in_blocks; ++i, addr += 0x1000) {
